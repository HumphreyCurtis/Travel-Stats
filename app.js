...
const compareBy = sort => (a,b) => sort.dir * String(a[sort.key] || '').localeCompare(String(b[sort.key] || ''));

function renderStats(){
  const continents = new Set(countries.map(country => country.continent.split(' ')[0].split('/')[0].replace(/[(),]/g,'')));
  const stats = [
    {num:countries.length,label:'Countries'},
    {num:`${Math.round(countries.length / 195 * 100)}%`,label:'of the world'},
    {num:cities.length,label:'Cities'},
    {num:continents.size,label:'Continents'}
  ];
  d3.select('#stats-strip').selectAll('.stat').data(stats).join('div').attr('class','stat')
    .html(item => `<span class="num">${item.num}</span><span class="label">${item.label}</span>`);
  d3.select('#footer-counts').text(`${countries.length} countries and ${cities.length} places logged`);
}

function renderCountries(filter=''){
  const query = filter.trim().toLowerCase();
  const data = countries.filter(country => country.name.toLowerCase().includes(query)).sort(compareBy(countrySort));
  d3.select('#countries-count').text(`${data.length} of ${countries.length} shown`);
  const rows = d3.select('#countries-table tbody').selectAll('tr').data(data,country => country.name).join('tr');
  ...
  rows.append('td').text(country => country.regions_visited?.join(', ') || '—');
}

function renderCities(filter=''){
  const query = filter.trim().toLowerCase();
  const data = cities.filter(city => city.name.toLowerCase().includes(query) || city.country.toLowerCase().includes(query)).sort(compareBy(citySort));
  d3.select('#cities-count').text(`${data.length} of ${cities.length} shown`);
  const rows = d3.select('#cities-table tbody').selectAll('tr').data(data,city => `${city.name}-${city.country}`).join('tr');
  rows.selectAll('*').remove();
  rows.append('td').text(city => city.name);
  rows.append('td').text(city => city.country);
  rows.append('td').each(function(city){
    if(city.notes) d3.select(this).append('span').attr('class','pill').text(city.notes);
  });
}

function bindTableControls(){
  d3.select('#country-search').on('input',function(){renderCountries(this.value);});
  d3.selectAll('[data-country-sort]').on('click',function(){
    d3.selectAll('[data-country-sort]').classed('active',false);d3.select(this).classed('active',true);
    countrySort={key:this.dataset.countrySort,dir:1};renderCountries(d3.select('#country-search').property('value'));
  });
  d3.selectAll('#countries-table th').on('click',function(){
    const key=this.dataset.key;countrySort={key,dir:countrySort.key===key ? -countrySort.dir : 1};
    renderCountries(d3.select('#country-search').property('value'));
  });
  d3.select('#city-search').on('input',function(){renderCities(this.value);});
  d3.selectAll('[data-city-sort]').on('click',function(){
    d3.selectAll('[data-city-sort]').classed('active',false);d3.select(this).classed('active',true);
    citySort={key:this.dataset.citySort,dir:1};renderCities(d3.select('#city-search').property('value'));
  });
  d3.selectAll('#cities-table th').on('click',function(){
    const key=this.dataset.key;citySort={key,dir:citySort.key===key ? -citySort.dir : 1};
...
function renderMap(world){
  ...
  const svg=d3.select('#map'),mapWrap=d3.select('#map-wrap'),tooltip=d3.select('#tooltip');
  const projection=d3.geoEquirectangular().fitExtent([[14,14],[width-14,height-14]],{type:'Sphere'});
  ...
  function showTooltip(event,title,lines=[]){
    ...
    tooltip.html('').style('left',`${x}px`).style('top',`${y}px`).style('opacity',1).attr('aria-hidden','false');
  ...
  layer.selectAll('path.country').data(features).join('path')
    .attr('class',feature => `country${visitedByISO.has(+feature.id) ? ' visited' : ''}`).attr('d',path)
  ...
  d3.select('#zoom-reset').on('click',() => svg.transition().duration(260).call(zoom.transform,d3.zoomIdentity));
}

async function init(){
  try{
    [countries,cities]=await Promise.all([d3.json(DATA_FILES.countries),d3.json(DATA_FILES.cities)]);
    renderStats();renderCountries();renderCities();bindTableControls();
    try{renderMap(await d3.json(DATA_FILES.world));}
    catch(error){d3.select('#load-error').classed('show',true).text('The map could not be loaded. The country and city tables are still available below.');console.error(error);}
  }catch(error){
    d3.select('#load-error').classed('show',true).text('The travel data could not be loaded. Run the site through a local web server or try again.');console.error(error);
  }
}
